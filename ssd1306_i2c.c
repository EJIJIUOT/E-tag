...
//#define PANEL0_TYPE SSD1306_128x32 ////128x64
#define PANEL0_TYPE SSD1306_74x40 ////128x64
//! @brief I2C address for panel 0
#define PANEL0_ADDR (0x3c<<1)//(0x3c<<1)
//! @brief If panel 0 has external RESET pin, define this as 1
#define PANEL0_RST 1 //res for TTGO oled

#define PANEL0_USE_RST 0
//! @brief GPIO MUX for panel 0 RESET pin
#define PANEL0_RST_MUX  PERIPHS_IO_MUX_GPIO16_U  //PERIPHS_IO_MUX_MTDI_U
//! @brief GPIO FUNC for panel 0 RESET pin
#define PANEL0_RST_FUNC FUNC_GPIO16_GPIO16 //FUNC_GPIO14
//! @brief GPIO bit location for panel 0 RESET pin
#define PANEL0_RST_BIT  BIT16//BIT12
...
#define SSD1306_74x40       3  //!< 74x40 panel

#define _RES1 gpio_set_level(g_reset_pin,1)
#define _RES0 gpio_set_level(g_reset_pin,0)

static uint8_t g_reset_pin;
...
bool ssd1306_init(uint8_t id,uint8_t scl_pin, uint8_t sda_pin,uint8_t reset_pin)
{  
 #if PANEL0_RST == 1   
   g_reset_pin = reset_pin;
   gpio_config_t io_conf;
    io_conf.intr_type = GPIO_INTR_DISABLE; //без прерываний
    io_conf.mode = GPIO_MODE_INPUT_OUTPUT; //выход
    io_conf.pin_bit_mask = (1ULL<<g_reset_pin); //бит маска
    io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE; //подтяжка к общему нет
    io_conf.pull_up_en = GPIO_PULLUP_ENABLE; //подтяжка к питанию ДА
    gpio_config(&io_conf);
...
//        memset(&(ctx->buffer),0,512);
        ctx->width = 128;
        ctx->height = 32;
    #elif (PANEL0_TYPE == SSD1306_74x40)
        ctx->type = SSD1306_74x40;
        ctx->buffer = malloc(370);  // 74 * 40 / 8
//        memset(&(ctx->buffer),0,512);
        ctx->width = 74;
        ctx->height = 40;   
  #else
...
//            dmsg_err_puts("Alloc OLED buffer failed.");
  ...
  #if PANEL0_USE_RST
        // Panel 0 reset
        gpio_set_level(RES_OLED ,1);
          vTaskDelay(50 / portTICK_PERIOD_MS);
          gpio_set_level(RES_OLED ,0);
        vTaskDelay(50 / portTICK_PERIOD_MS);
        gpio_set_level(RES_OLED ,1);
...
//        dmsg_err_puts("OLED I2C bus not responding.");
    ...
    else if (ctx->type == SSD1306_128x32)
    {
     ...
     else if (ctx->type == SSD1306_74x40)
    {
        _command(ctx->address, 0xae); // SSD1306_DISPLAYOFF
        _command(ctx->address, 0xd5); // SSD1306_SETDISPLAYCLOCKDIV   
        _command(ctx->address, 0x80); // Suggested value 0x80
        _command(ctx->address, 0xa8); // SSD1306_SETMULTIPLEX
        _command(ctx->address, 0x27); // 1/40//27
        _command(ctx->address, 0xad); //  Internal IREF Setting for the 0.42 OLED
        _command(ctx->address, 0x30);
        _command(ctx->address, 0x8d); // SSD1306_CHARGEPUMP
        _command(ctx->address, 0x15); // Charge pump on 
        _command(ctx->address, 0x40); // SSD1306_SETSTARTLINE line #0
        _command(ctx->address, 0xd3); // SSD1306_SETDISPLAYOFFSET
        _command(ctx->address, 0x00); // 0 no offset
       _command(ctx->address, 0xa6); // SSD1306_NORMALDISPLAY 
       _command(ctx->address, 0xa4); // output ram to display 
        _command(ctx->address, 0x20); // SSD1306_MEMORYMODE horizontal addressing mode
        _command(ctx->address, 0x00); // 0x0 act like ks0108 
        //_command(ctx->address, 0xa1); // SSD1306_SEGREMAP | 1 незеркально
        _command(ctx->address, 0xc8); // SSD1306_COMSCANDEC
        _command(ctx->address, 0xa0); // SSD1306_SEGREMAP | 1 зеркальное
         _command(ctx->address, 0xda); // SSD1306_SETCOMPINS
      //  _command(ctx->address, 0x02);
        _command(ctx->address, 0x12);
        //_command(ctx->address, 0x81); // SSD1306_SETCONTRAST
      ...
      //  _command(ctx->address, 0xf1);
         _command(ctx->address, 0xf1);
        _command(ctx->address, 0x2e); // SSD1306_DEACTIVATE_SCROLL
    //  _command(ctx->address, 0xaf); // SSD1306_DISPLAYALLON_RESUME 
...
void ssd1306_refresh(uint8_t id, bool force)
{
    ...
    if (force)
    {
        ...
        {
            _command(ctx->address, 0x21); // SSD1306_COLUMNADDR
            _command(ctx->address, 28);    // column start
            _command(ctx->address, 101);  // column end
            _command(ctx->address, 0x22); // SSD1306_PAGEADDR
            _command(ctx->address, 0);    // page start
            _command(ctx->address, 5);    // page end (5 pages for 40 rows OLED)
            for (k = 0; k < 370; k++)
            {
                i2c_start();
                i2c_write(ctx->address);
...
void ssd1306_drawBitmap(uint8_t id,int8_t x, int8_t y, const uint8_t *picture, uint8_t width, uint8_t height, uint8_t color){

    uint8_t i,j, byteWidth = (width+7)/8;
    for (j = 0; j < height; j++) {
        for(i=0; i < width;i++){
            if(pgm_read_byte(picture + j * byteWidth + i / 8) & (128 >> (i & 7))) {
                ssd1306_draw_pixel(id,x+i, y+j, color);
            } else {
                ssd1306_draw_pixel(id,x+i, y+j, !color);
            }
        }
